...
show_help() {
  echo -e \
"Options:
-B, --blockupdates     : block client auto-updates
-c, --clearcache       : clear client app cache
-d, --devmode          : enable developer mode
--dry-run              : preview which patches would apply (no changes made)
...
--list                 : list all patch names and exit
--list-backups         : list available backups and exit
-l, --lyricsbg         : set lyrics background color to black
--lyrics-color=<hex>   : set lyrics background color (e.g. --lyrics-color=#101010)
--lyrics-text=<hex>    : set lyrics active text color
--mirror=<url>         : fallback download mirror base URL
--no-about             : skip injecting the About panel
--nocolor              : remove colors from SpotX-Bash output
-o, --oldui            : use old home screen UI
--only=<a,b>           : apply only the named patches
-p, --premium          : paid premium-tier subscriber
-P <path>              : set path to client
--patches=<file>       : load patch definitions from a local file
--quiet                : suppress decorative output (errors still shown)
--restore[=<ts>]       : restore client from canonical or timestamped backup
...
--skip=<a,b>           : skip the named patches
--stable               : use with '--installdeb' for stable branch [Linux]
--start                : launch Spotify after patching
--autoreapply          : auto-reapply SpotX after client updates
--no-autoreapply       : remove the auto-reapply watcher
...
# --- output mode + JSON summary -------------------------------------------
jsonKeys=(); jsonVals=()
json_set() { jsonKeys+=("$1"); jsonVals+=("$2"); }
json_emit() {
  local i out=""
  for i in "${!jsonKeys[@]}"; do
    [[ -n "${out}" ]] && out+=","
    out+="\"${jsonKeys[$i]}\":\"${jsonVals[$i]//\"/\\\"}\""
  done
  printf '{%s}\n' "${out}"
}
ok() {      # ok "<message>" -> "✔ message" (suppressed when --quiet/--json)
  [[ "${quiet}" || "${jsonOut}" ]] && return 0
  printf '\xE2\x9C\x94\x20%b\n' "$1"
}
status() {  # status "<line>" -> plain line (suppressed when --quiet/--json)
  [[ "${quiet}" || "${jsonOut}" ]] && return 0
  printf '%b\n' "$1"
}

# --- lyrics theme defaults (overridable via flags/config) -----------------
lyricsBgColor='#121212'
lyricsActive='#C8C8C8'
lyricsInactive='#575757'

# --- patch filtering + tally ----------------------------------------------
skipList=''
onlyList=''
patchApplied=0
patchMissing=0
...
# --- config file ----------------------------------------------------------
configDir="${XDG_CONFIG_HOME:-$HOME/.config}/spotx-bash"
...
load_config() {
  ...
  while IFS='=' read -r key val; do
    ...
    case "${key}" in
      ...
      start) startSpotify='true' ;;
      autoreapply) autoReapply='true' ;;
      verbose) verbose='true' ;;
      quiet) quiet='true' ;;
      json) jsonOut='true' ;;
      interactive) interactiveMode='true' ;;
      force) forceSpotx='true' ;;
      lyrics_color) lyricsBgColor="${val}"; lyricsBg='true' ;;
      lyrics_text) lyricsActive="${val}"; lyricsBg='true' ;;
      mirror) mirrorUrl="${val}" ;;
      patches) patchesFile="${val}" ;;
      skip) skipList="${skipList} ${val//,/ }" ;;
      only) onlyList="${onlyList} ${val//,/ }" ;;
      *) ;;
    esac
  done < "${configFile}"
}
save_config() {
  mkdir -p "${configDir}"
  {
    echo "# SpotX-Bash config (generated by --save-config)"
    [[ "${paidPremium}" ]] && echo "premium=true"
    [[ "${hideNonMusic}" ]] && echo "hide=true"
    [[ "${devMode}" ]] && echo "devmode=true"
    [[ "${excludeExp}" ]] && echo "noexp=true"
    [[ "${oldUi}" ]] && echo "oldui=true"
    [[ "${clearCache}" ]] && echo "clearcache=true"
    [[ "${blockUpdates}" ]] && echo "blockupdates=true"
    [[ "${lyricsBg}" ]] && echo "lyricsbg=true"
    [[ "${skipCodesign}" ]] && echo "skipcodesign=true"
    [[ "${startSpotify}" ]] && echo "start=true"
    [[ "${autoReapply}" ]] && echo "autoreapply=true"
    [[ "${verbose}" ]] && echo "verbose=true"
    [[ "${lyricsBgColor}" != '#121212' ]] && echo "lyrics_color=${lyricsBgColor}"
    [[ "${lyricsActive}" != '#C8C8C8' ]] && echo "lyrics_text=${lyricsActive}"
    [[ -n "${mirrorUrl}" ]] && echo "mirror=${mirrorUrl}"
    [[ -n "${patchesFile}" ]] && echo "patches=${patchesFile}"
  } > "${configFile}"
  status "${green}Saved config to ${configFile}${clr}"
}
...
while getopts ':BcdefF:hilopP:SvV:-:' flag; do
  case "${flag}" in
    -)
      case "${OPTARG}" in
        autoreapply) autoReapply='true' ;;
        no-autoreapply) autoReapplyOff='true' ;;
        blockupdates) blockUpdates='true' ;;
        clearcache) clearCache='true' ;;
        debug) debug='true' ;;
        devmode) devMode='true' ;;
        ...
        interactive) interactiveMode='true' ;;
        json) jsonOut='true' ;;
        list) listPatches='true' ;;
        list-backups) listBackups='true' ;;
        logo) logoVar='true' ;;
        lyricsbg) lyricsBg='true' ;;
        lyrics-color=*) lyricsBgColor="${OPTARG#*=}"; lyricsBg='true' ;;
        lyrics-text=*) lyricsActive="${OPTARG#*=}"; lyricsBg='true' ;;
        mirror=*) mirrorUrl="${OPTARG#*=}" ;;
        no-about) skipList="${skipList} aboutSpotX" ;;
        nocolor) unset clr green red yellow ;;
        noexp) excludeExp='true' ;;
        oldui) oldUi='true' ;;
        only=*) onlyArg="${OPTARG#*=}"; onlyList="${onlyList} ${onlyArg//,/ }" ;;
        patches=*) patchesFile="${OPTARG#*=}" ;;
        premium) paidPremium='true' ;;
        quiet) quiet='true' ;;
        restore) restoreReq='true' ;;
        restore=*) restoreReq='true'; restoreTs="${OPTARG#*=}" ;;
        rollback) [[ "${platformType}" == "macOS" ]] && rollback='true'; installMac='true' ;;
        save-config) saveConfig='true' ;;
        skip=*) skipArg="${OPTARG#*=}"; skipList="${skipList} ${skipArg//,/ }" ;;
...
# In quiet/json mode, send decorative stdout to /dev/null (errors stay on stderr);
# fd 3 holds the real stdout so JSON can be emitted at the end.
[[ "${quiet}" || "${jsonOut}" ]] && exec 3>&1 1>/dev/null
...
linux_no_client() {
  { [[ -e /etc/NIXOS ]] || grep -qi '^ID=nixos' /etc/os-release 2>/dev/null; } && {
    echo -e "${red}Error:${clr} NixOS detected. A Spotify install under the read-only /nix/store cannot be patched in place.\nUse a writable copy (e.g. home-manager into ~/.local, or copy the app to a writable dir) and re-run with '-P <path>'.\nSee the SpotX-Bash FAQ for NixOS guidance.\n" >&2
    exit 1
  }
...
linux_prepare() {
  case "$(uname -m)" in
    aarch64|arm64) archVar="arm64" ;;
    *) archVar="x86_64" ;;
  esac
  [[ "${archVar}" != "x86_64" ]] && echo -e "${yellow}Warning:${clr} ${archVar} detected; binary-level patches may not apply (JS/CSS patches still apply).\n" >&2
  linux_set_path
  appPath="${installPath}"
...
perlvar() {
  { local e pc; [[ "${dryRun}" ]] && pc="perl -0777 -w -ne" || pc="${perlVar}"
    e=$(${pc} 'BEGIN { $m = 0; $c = 0 } $c += s&'"${a[1]}"'&'"${a[2]}"'&'"${a[3]}"' and $m = 1; END { print "$m,$c" }' "${p}")
    local s="$?"
    local m=$(echo "${e}" | cut -d',' -f1)
    local c=$(echo "${e}" | cut -d',' -f2)
    [[ "${m}" == 1 ]] && patchApplied=$((patchApplied+1)) || patchMissing=$((patchMissing+1))
    [[ "${dryRun}" ]] && { [[ "${m}" == 1 ]] && printf '  %bwould apply%b %s (%sx)\n' "${green}" "${clr}" "${a[0]}" "${c}" || printf '  %bwould MISS %b %s\n' "${red}" "${clr}" "${a[0]}"; return; }
...
linux_deb_install() {
  ...
  lc02=$(printf "%s" \
    ...
    | rev | base64 --decode | base64 --decode)
  if [[ -n "${mirrorUrl}" ]]; then
    curl -q --progress-bar -f -L --retry 5 -o "${workDir}/${fileVar}" "${mirrorUrl%/}/${fileVar}" || {
      rm -f "${workDir}/${fileVar}" 2>/dev/null
      echo -e "\n${red}Error:${clr} Mirror download failed\n" >&2; exit 1
    }
  else
    eval "${lc01}"; eval "${lc02}"
  fi
...
macos_client_install() {
  ...
  mc02=$(printf "%s" \
    ...
    | rev | base64 --decode | base64 --decode)
  if [[ -n "${mirrorUrl}" ]]; then
    curl -q --progress-bar -f -L --retry 5 -o "$HOME/Downloads/${fileVar}" "${mirrorUrl%/}/${fileVar}" || {
      rm -f "$HOME/Downloads/${fileVar}" 2>/dev/null
      echo -e "\n${red}Error:${clr} Mirror download failed\n" >&2; exit 1
    }
  else
    eval "${mc01}"; eval "${mc02}"
...
perlVar() {
  ...
  for cmd in "${A[@]}"; do
    IFS='&' read -r -a a <<< "${cmd}"
    [[ -n "${onlyList}" && " ${onlyList} " != *" ${a[0]} "* ]] && continue
    [[ -n "${skipList}" && " ${skipList} " == *" ${a[0]} "* ]] && continue
    local f="${a[4]}"
    local p="${!f}"
...
snapshot_check() {
  ...
  [[ ! -f "${xpuiJs}" ]] && [[ -f "${xpuiSnapshotJs}" ]] && {
    ...
    ' "${snapshotBinary}" "${START_XM}" "${END_XM}" "${xpuiSnapshotJs}" || {
      [[ "${dryRun}" ]] && { echo -e "\n${yellow}Warning:${clr} Snapshot processing failed (dry-run)\n" >&2; return 1; }
      uninstall_spotx
...
xpui_open() {
  ...
  [[ "${versionFailed}" && -z "${forceVer+x}" || -z "${forceVer+x}" && ( "${debug}" && "${devMode}" && "${t}" || "${verbose}" ) ]] && {
    ...
    [[ -z "${unknownVer+x}" ]] && (( $(ver "${clientVer}") <= $(ver "${sxbVer}") && $(ver "${clientVer}") > $(ver "0") )) && ok "Detected Spotify ${green}${clientVer}${clr}"
    [[ -z "${unknownVer+x}" ]] && (( $(ver "${clientVer}") > $(ver "${sxbVer}") )) && ok "Detected Spotify ${red}${clientVer}${clr}"
  }
...
run_core_start() {
  ...
  check_write_permission "${appPath}" "${appBinary}" "${xpuiPath}" "${xpuiSpa}"
  xpui_detect
  [[ "${xpuiSkip}" ]] && { ok "Finished"; status ""; exit 0; }
  xpui_open
...
run_patches() {
  ...
  [[ "${excludeExp}" ]] && ok "Skipped experimental features" || {
    ...
    ok "Enabled experimental features"
  }
  [[ "${oldUi}" ]] && {
    perlVar "${oldUiEx[@]}"
    (($(ver "${clientVer}") >= $(ver "1.1.93.896") && $(ver "${clientVer}") <= $(ver "1.2.13.661"))) && ok "Enabled old UI"
  ...
  [[ -z "${oldUi+x}" ]] && (($(ver "${clientVer}") >= $(ver "1.1.93.896"))) && {
    perlVar "${newUiEx[@]}"
    (($(ver "${clientVer}") <= $(ver "1.2.13.661"))) && ok "Enabled new UI"
  }
  [[ "${hideNonMusic}" ]] && (($(ver "${clientVer}") >= $(ver "1.1.70.610"))) && {
    perlVar "${podEx[@]}"
    (($(ver "${clientVer}") >= $(ver "1.2.45.451"))) && printf '\n%s' "${hideSubfeed}" >> "${xpuiCss}"
    ok "Removed non-music categories on home screen"
  }
  [[ "${lyricsBg}" ]] && {
    (($(ver "${clientVer}") >= $(ver "1.2.0.1165"))) && {
      perlVar "${lyricsBgEx[@]}"
      (($(ver "${clientVer}") >= $(ver "1.2.45.454"))) && printf '\n%b' "${lyricsBackgroundNew}" >> "${xpuiCss}"
      ok "Enabled custom background for lyrics"
    } || {
      echo -e "\n${yellow}Warning:${clr} Custom lyrics background is not supported in this version...\n" >&2
    }
  }
  [[ "${blockUpdates}" && "${platformType}" == "macOS" ]] && {
    perlVar "${updatesEx[@]}"
    ok "Blocked automatic updates"
  }
}
...
list_patches() {
  local cat ref name e
  for cat in aoEx freeEx devEx expEx premiumExpEx oldUiEx newUiEx podEx lyricsBgEx updatesEx; do
    printf '\n[%s]\n' "${cat}"
    ref="${cat}[@]"
    for e in "${!ref}"; do
      name="${e%%&*}"
...
load_patches_file() {
  local file="$1" line cat=""
  [[ -f "${file}" ]] || { echo -e "${red}Error:${clr} patches file not found: ${file}\n" >&2; exit 1; }
  local cAo cFree cDev cExp cPrem cOld cNew cPod cLyr cUpd
  while IFS= read -r line || [[ -n "${line}" ]]; do
    line="${line%$'\r'}"
    [[ -z "${line}" || "${line}" == \#* ]] && continue
    [[ "${line}" == \[*\] ]] && { cat="${line#[}"; cat="${cat%]}"; continue; }
    case "${cat}" in
      aoEx) [[ -z "${cAo}" ]] && { aoEx=(); cAo=1; }; aoEx+=("${line}") ;;
      freeEx) [[ -z "${cFree}" ]] && { freeEx=(); cFree=1; }; freeEx+=("${line}") ;;
      devEx) [[ -z "${cDev}" ]] && { devEx=(); cDev=1; }; devEx+=("${line}") ;;
      expEx) [[ -z "${cExp}" ]] && { expEx=(); cExp=1; }; expEx+=("${line}") ;;
      premiumExpEx) [[ -z "${cPrem}" ]] && { premiumExpEx=(); cPrem=1; }; premiumExpEx+=("${line}") ;;
      oldUiEx) [[ -z "${cOld}" ]] && { oldUiEx=(); cOld=1; }; oldUiEx+=("${line}") ;;
      newUiEx) [[ -z "${cNew}" ]] && { newUiEx=(); cNew=1; }; newUiEx+=("${line}") ;;
      podEx) [[ -z "${cPod}" ]] && { podEx=(); cPod=1; }; podEx+=("${line}") ;;
      lyricsBgEx) [[ -z "${cLyr}" ]] && { lyricsBgEx=(); cLyr=1; }; lyricsBgEx+=("${line}") ;;
      updatesEx) [[ -z "${cUpd}" ]] && { updatesEx=(); cUpd=1; }; updatesEx+=("${line}") ;;
      *) ;;
    esac
  done < "${file}"
  status "${green}Loaded patch definitions from ${file}${clr}"
}

run_dryrun() {
  final_setup_check
  local tmpd; tmpd=$(mktemp -d 2>/dev/null) || { echo -e "${red}Error:${clr} could not create temp dir\n" >&2; exit 1; }
  unzip -qq "${xpuiSpa}" -d "${tmpd}" || { rm -rf "${tmpd}"; echo -e "${red}Error:${clr} could not unpack xpui.spa\n" >&2; exit 1; }
  xpuiDir="${tmpd}"
  derive_xpui_paths
  snapshot_check || true
  local dv; dv=$(perl -ne '/[Vv]ersion[:=,\x22]{1,3}(1\.[0-9]+\.[0-9]+\.[0-9]+)\.g[0-9a-f]+/ && print "$1"' "${xpuiJs}" 2>/dev/null)
  [[ -n "${dv}" ]] && clientVer="${dv}"
  (($(ver "${clientVer}") > $(ver "1.2.56.9999"))) && vendorXpuiJs="${xpuiJs}"
  echo -e "Dry run — client ${clientVer:-unknown} (${platformType}/${archVar}); no changes will be made.\n"
  dryRun='true'
  printf '[aoEx]\n';            perlVar "${aoEx[@]}"
  [[ "${paidPremium}" ]] || { printf '[freeEx]\n'; perlVar "${freeEx[@]}"; }
  printf '[expEx]\n';           perlVar "${expEx[@]}"
  [[ "${paidPremium}" ]] && { printf '[premiumExpEx]\n'; perlVar "${premiumExpEx[@]}"; }
  printf '[devEx]\n';           perlVar "${devEx[@]}"
  printf '[oldUiEx/newUiEx]\n'; perlVar "${oldUiEx[@]}"; perlVar "${newUiEx[@]}"
  printf '[podEx]\n';           perlVar "${podEx[@]}"
  printf '[lyricsBgEx]\n';      perlVar "${lyricsBgEx[@]}"
  printf '[updatesEx]\n';       perlVar "${updatesEx[@]}"
  echo -e "\nWould apply: ${green}${patchApplied}${clr} | would miss: ${red}${patchMissing}${clr}\n"
  rm -rf "${tmpd}"
}

backup_timestamped() {
  local ts; ts=$(date +%Y%m%d%H%M%S)
  [[ -f "${xpuiBak}" ]] && cp "${xpuiBak}" "${xpuiPath}/xpui.spa.bak.${ts}" 2>/dev/null
  [[ -f "${appBak}" ]] && cp "${appBak}" "${appBinary}.bak.${ts}" 2>/dev/null
  ls -1t "${xpuiPath}"/xpui.spa.bak.* 2>/dev/null | tail -n +4 | while read -r f; do rm -f "${f}"; done
  ls -1t "${appBinary}".bak.* 2>/dev/null | tail -n +4 | while read -r f; do rm -f "${f}"; done
}

list_backups() {
  local f ts
  echo -e "Backups in ${installOutput}:\n"
  [[ -f "${xpuiBak}" ]] && echo "  canonical              xpui.bak"
  for f in "${xpuiPath}"/xpui.spa.bak.*; do
    [[ -e "${f}" ]] || continue
    ts="${f##*.}"
    echo "  ${ts:0:4}-${ts:4:2}-${ts:6:2} ${ts:8:2}:${ts:10:2}:${ts:12:2}    ${f##*/}"
  done
  echo
}
...
do_restore() {
  local src bsrc
  if [[ -n "${restoreTs}" ]]; then
    src="${xpuiPath}/xpui.spa.bak.${restoreTs}"; bsrc="${appBinary}.bak.${restoreTs}"
    [[ -f "${src}" ]] || { echo -e "${red}Error:${clr} no backup found for ${restoreTs}\n" >&2; exit 1; }
  else
    src="${xpuiBak}"; bsrc="${appBak}"
    [[ -f "${src}" ]] || { echo -e "${red}Error:${clr} no canonical backup found\n" >&2; exit 1; }
  fi
  check_write_permission "${appPath}" "${appBinary}" "${xpuiPath}" "${xpuiSpa}"
  rm -f "${xpuiSpa}"; cp "${src}" "${xpuiSpa}"
  [[ -f "${bsrc}" ]] && { rm -f "${appBinary}"; cp "${bsrc}" "${appBinary}"; }
  [[ "${platformType}" == "macOS" ]] && { /usr/bin/xattr -cr "${appPath}" 2>/dev/null; codesign -f --deep -s - "${appPath}" 2>/dev/null; }
  ok "Restored client from ${restoreTs:-canonical} backup"
}

start_spotify() {
  [[ "${platformType}" == "macOS" ]] && { open -a Spotify >/dev/null 2>&1 &
  } || { [[ "${flatpakClient}" ]] && { flatpak run com.spotify.Client >/dev/null 2>&1 &
  } || { setsid "${appBinary}" >/dev/null 2>&1 & }; }
  ok "Launched Spotify"
}

verify_patch() {
  [[ -f "${xpuiSpa}" ]] || return 0
  if unzip -p "${xpuiSpa}" 2>/dev/null | grep -Fq "SpotX was here"; then
    ok "Verified patch (SpotX marker present)"
  else
    echo -e "${yellow}Warning:${clr} Patch marker not found after repackage; verification failed.\n" >&2
  fi
}

linux_block_updates() {
  [[ "${flatpakClient}" ]] && {
    command -v flatpak >/dev/null && flatpak mask com.spotify.Client >/dev/null 2>&1 && ok "Masked flatpak updates for Spotify" \
      || echo -e "${yellow}Warning:${clr} Could not mask flatpak updates.\n" >&2
    return
  }
  command -v apt-mark >/dev/null && {
    sudo apt-mark hold spotify-client >/dev/null 2>&1 && ok "Held spotify-client package (apt-mark hold)" \
      || echo -e "${yellow}Warning:${clr} Could not 'apt-mark hold spotify-client'.\n" >&2
    return
  }
  echo -e "${yellow}Warning:${clr} No supported Linux update-block method (apt/flatpak) found.\n" >&2
}

autoreapply_remove() {
  if [[ "${platformType}" == "macOS" ]]; then
    local plist="${HOME}/Library/LaunchAgents/com.spotx.reapply.plist"
    launchctl unload "${plist}" 2>/dev/null
    rm -f "${plist}"
  else
    local ud="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user"
    command -v systemctl >/dev/null && systemctl --user disable --now spotx-reapply.path >/dev/null 2>&1
    rm -f "${ud}/spotx-reapply.path" "${ud}/spotx-reapply.service"
    command -v systemctl >/dev/null && systemctl --user daemon-reload 2>/dev/null
  fi
  rm -f "${configDir}/reapply.sh" "${configDir}/last-reapply"
  ok "Removed auto-reapply watcher"
}

autoreapply_install() {
  local self; self=$(realpath "$0" 2>/dev/null || echo "$0")
  mkdir -p "${configDir}"
  local wrapper="${configDir}/reapply.sh"
  if [[ "${platformType}" == "macOS" ]]; then
    {
      printf '#!/usr/bin/env bash\n'
      printf 'v=$(defaults read "%s/Contents/Info.plist" CFBundleShortVersionString 2>/dev/null)\n' "${appPath}"
      printf 'marker="%s/last-reapply"\n' "${configDir}"
      printf '[[ -f "$marker" && "$(cat "$marker")" == "$v" ]] && exit 0\n'
      printf '/usr/bin/env bash "%s" --quiet && printf "%%s" "$v" > "$marker"\n' "${self}"
    } > "${wrapper}"
    chmod +x "${wrapper}"
    defaults read "${appPath}/Contents/Info.plist" CFBundleShortVersionString 2>/dev/null > "${configDir}/last-reapply"
    local plist="${HOME}/Library/LaunchAgents/com.spotx.reapply.plist"
    mkdir -p "${HOME}/Library/LaunchAgents"
    {
      ...
      printf '  <key>Label</key><string>com.spotx.reapply</string>\n'
      printf '  <key>ProgramArguments</key><array><string>/usr/bin/env</string><string>bash</string><string>%s</string></array>\n' "${wrapper}"
      printf '  <key>WatchPaths</key><array><string>%s</string></array>\n' "${appBinary}"
      printf '  <key>RunAtLoad</key><false/>\n'
      printf '</dict></plist>\n'
    } > "${plist}"
    launchctl unload "${plist}" 2>/dev/null
    launchctl load "${plist}" 2>/dev/null && ok "Installed auto-reapply LaunchAgent" \
      || echo -e "${yellow}Warning:${clr} Could not load LaunchAgent.\n" >&2
    return
  fi
  command -v systemctl >/dev/null || { echo -e "${yellow}Warning:${clr} systemd --user unavailable; cannot install auto-reapply watcher.\n" >&2; return; }
  {
    printf '#!/usr/bin/env bash\n'
    printf 'v=$("%s" --version 2>/dev/null)\n' "${appBinary}"
    printf 'marker="%s/last-reapply"\n' "${configDir}"
    printf '[[ -f "$marker" && "$(cat "$marker")" == "$v" ]] && exit 0\n'
    printf '/usr/bin/env bash "%s" --quiet && printf "%%s" "$v" > "$marker"\n' "${self}"
  } > "${wrapper}"
  chmod +x "${wrapper}"
  "${appBinary}" --version 2>/dev/null > "${configDir}/last-reapply"
  local ud="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user"
  mkdir -p "${ud}"
  {
    printf '[Unit]\nDescription=Reapply SpotX-Bash after Spotify update\n\n'
    printf '[Service]\nType=oneshot\nExecStart=/usr/bin/env bash %s\n' "${wrapper}"
  } > "${ud}/spotx-reapply.service"
  {
    printf '[Unit]\nDescription=Watch Spotify for updates (SpotX-Bash)\n\n'
    printf '[Path]\nPathChanged=%s\nUnit=spotx-reapply.service\n\n' "${appBinary}"
    printf '[Install]\nWantedBy=default.target\n'
  } > "${ud}/spotx-reapply.path"
  systemctl --user daemon-reload 2>/dev/null
  systemctl --user enable --now spotx-reapply.path >/dev/null 2>&1 && ok "Installed auto-reapply watcher (systemd --user)" \
    || echo -e "${yellow}Warning:${clr} Could not enable systemd path unit.\n" >&2
}

# ----------------------------- main flow ----------------------------------
[[ "${listPatches}" ]] && { list_patches; exit 0; }
[[ "${autoReapplyOff}" ]] && { autoreapply_remove; exit 0; }
[[ -n "${patchesFile}" ]] && load_patches_file "${patchesFile}"

run_prepare

[[ "${listBackups}" ]] && { list_backups; exit 0; }
[[ "${restoreReq}" ]] && { do_restore; exit 0; }
[[ "${dryRun}" ]] && { run_dryrun; exit 0; }

run_uninstall_check
run_interactive_check
run_install_check
run_cache_check
[[ "${blockUpdates}" && "${platformType}" == "Linux" ]] && linux_block_updates
run_core_start
backup_timestamped
run_patches
run_finish
verify_patch
[[ "${startSpotify}" ]] && start_spotify
[[ "${autoReapply}" ]] && autoreapply_install

[[ "${jsonOut}" ]] && exec 1>&3
[[ "${jsonOut}" ]] && {
  json_set version "${clientVer}"
  json_set path "${installOutput}"
  json_set applied "${patchApplied}"
  json_set missing "${patchMissing}"
  json_set result "finished"
  json_emit
} || { ok "Finished"; status ""; }
exit 0
